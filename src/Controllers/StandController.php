...
class StandController extends Controller
{
    ...
    public function indexCart()
    {
        ...
        if(Session::has('cart')) {
            ...
            foreach($list as $l) {
                ...
                if($l->is_variant) {
                    $furnishing_data = DB::table('furnishings')
                        ->leftJoin('furnishings_translations', 'furnishings.variant_id', '=', 'furnishings_translations.furnishing_id')
                        ->where([
                            ['furnishings.id', '=', $l->furnishing_id],
                            ['furnishings_translations.locale', '=', App::getLocale()]
                        ])
                ...
                $module_data = DB::table('code_modules')->where('id', '=', $l->code_module)->first();
                $l->module = $module_data->code;

                $relationship = DB::table('furnishings_stands_types')->where([
                    ['furnishing_id', '=', $l->furnishing_id],
                    ['stand_type_id', '=', $module_data->stand_type_id]
                ])->first();
                if($l->is_variant) {
                    $relationship = DB::table('furnishings_stands_types')->where([
                        ['furnishing_id', '=', $l->variant_id],
                        ['stand_type_id', '=', $module_data->stand_type_id]
                    ])->first();
                }

                $l->is_supplied = $relationship->is_supplied;

                $l->price = 0;
                if($furnishing_data->extra_price) {
                    $l->price = $furnishing_data->price * $l->qty;
                } else {
                    if($l->is_supplied) {
                        if($l->qty > $relationship->max) {
                            $diff = $l->qty - $relationship->max;
                            $l->price = $furnishing_data->price * $diff;
                        }
                    } else {
                        $l->price = $furnishing_data->price * $l->qty;
                    }
                ...
                $l->description = $furnishing_data->description;
            }
        }

        Session::put('total', $total);
        
        return view('admin.cart', ['list' => $list, 'total' => $total]);
    }
    ...
    public function checkItemInCart(Request $request)
    {
        ...
        try {
            ...
            $response['check'] = checkItemInCart($request->code_module, $request->id);
            $response['status'] = true;
            return response()->json($response);
        } catch(\Exception $e){
            $response['message'] = $e->getMessage();
    ...
    public function addToCart(Request $request)
    {
        ...
        try {
            $validation_data = [
                'id' => ['required', 'exists:furnishings,id'],
                'code_module' => ['required', 'exists:code_modules,id'],
            ...
            $obj_cart = new stdClass();
            $obj_cart->code_module = $request->code_module;
            $obj_cart->furnishing_id = $request->id;
            $obj_cart->qty = $request->qty;
            $obj_cart->color = $request->color;
            $obj_cart->is_variant = $request->is_variant;
            $obj_cart->variant_id = $request->is_variant ? $furnishing_id : null;
            
            array_push($cart, $obj_cart);
            Session::put('cart', $cart);

            $furnishing_translation = DB::table('furnishings_translations')->where([
                ['furnishing_id', '=', $furnishing_id],
                ['locale', '=', App::getLocale()]
            ])->first();

            $response['status'] = true;
            $response['message'] = trans('generals.added_to_cart', ['el' => $furnishing_translation->description]);
            $response['count'] = count(Session::get('cart'));
            return response()->json($response);
        } catch(\Exception $e){
            $response['message'] = $e->getMessage();
            return response()->json($response);
    ...
    public function removeToCart(Request $request)
    {
        ...
        try {
            ...
            $cart = Session::get('cart');
            foreach($cart as $i => $c) {
                if($c->code_module == $request->code_module && $c->furnishing_id == $request->id) {
                    unset($cart[$i]);
                }
            }
            Session::put('cart', $cart);
            ...
            $response['status'] = true;
            $response['message'] = trans('generals.removed_to_cart', ['el' => $furnishing_translation->description]);
            $response['count'] = Session::has('cart') ? count(Session::get('cart')) : '';
            return response()->json($response);
        } catch(\Exception $e){
            $response['message'] = $e->getMessage();
            return response()->json($response);
    ...
    public function confirmCart(Request $request)
    {
        ...
        try {
            ...
            foreach($cart as $c) {
                if($c->is_supplied) {
                    $obj = new stdClass();
                    $obj->furnishing_id = $c->furnishing_id;
                    array_push($furnishing_objs, $obj);
                    array_push($furnishing_ids, $c->furnishing_id);
                    array_push($code_modules, $c->code_module);
                }
            }

            $list_supplied_by_stand = DB::table('furnishings_stands_types')
                ->leftJoin('code_modules', 'furnishings_stands_types.stand_type_id', '=', 'code_modules.stand_type_id')
                ->whereIn('code_modules.id', $code_modules)
                ->where([
                    ['furnishings_stands_types.is_supplied', '=', 1],
                    ['furnishings_stands_types.min', '>', 0]
                ])
                ->select('furnishings_stands_types.furnishing_id')
                ->get()
                ->toArray();

            $not_inserted = array_udiff($list_supplied_by_stand, $furnishing_objs,
                function ($obj_a, $obj_b) {
                    return strcmp($obj_a->furnishing_id, $obj_b->furnishing_id);
                }
            );

            if(count($not_inserted) > 0) {
                $response['message'] = trans('api.check_supplied_cart');
                return response()->json($response);
            }

            // get data of exhibitor authenticated
            $exhibitor_data = DB::table('exhibitors_data')->where('email_responsible', '=', auth()->user()->email)->first();

            $exhibitor = Exhibitor::findOrFail($exhibitor_data->exhibitor_id);

            // track/save order somewhere
            foreach($cart as $c) {
                Order::create([
                    'exhibitor_id' => $exhibitor_data->exhibitor_id,
                    'code_module_id' => $c->code_module,
                    'furnishing_id' => $c->furnishing_id,
                    'qty' => $c->qty,
                    'is_supplied' => $c->is_supplied,
                    'created_at' => DB::raw('NOW()'),
                    'updated_at' => DB::raw('NOW()')
                ]);
            }
            ...
            /*
            $furnishings = DB::table('furnishings_translations')
                ->leftJoin('furnishings', 'furnishings_translations.furnishing_id', '=', 'furnishings.id')
                ->whereIn('furnishings_translations.furnishing_id', $furnishing_ids)
                ->where('furnishings_translations.locale', '=', $exhibitor_data->locale)
                ->select('furnishings.color', 'furnishings_translations.*')
                ->get();
            */
            ...
            $furnishings = DB::table('furnishings')
                ...
                ->whereIn('furnishings.id', $furnishing_ids)
                ->where('furnishings_translations.locale', '=', $exhibitor->locale)
                ->select('furnishings.*', 'furnishings_translations.description')
                ->get();
            ...
            foreach($furnishings as $furnishing) {
                $furnishings_txt .= '<li>'.$furnishing->description.'</li>';
            }
            $furnishings_txt .= '</ul>';

            $body = formatDataForEmail([
                'orders' => $furnishings_txt,
                'tot' => Session::get('total'),
            ], $exhibitor->locale == 'it' ? $setting->email_confirm_order_it : $setting->email_confirm_order_en);
            ...
            // send email to benedetta.lazzari@equipemilano.com
            //$furnishings = DB::table('furnishings_translations')->whereIn('furnishing_id', $furnishing_ids)->where('locale', '=', 'it')->get();
            /*$furnishings = DB::table('furnishings_translations')
                ->leftJoin('furnishings', 'furnishings_translations.furnishing_id', '=', 'furnishings.id')
                ->whereIn('furnishings_translations.furnishing_id', $furnishing_ids)
                ->where('furnishings_translations.locale', '=', 'it')
